    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float distanceField(float3 pos)
            {
                ...
                //float box1 = sdRoundBox(pos - _box1.xyz, _box1.www, _box1round);
                //float4 c = map(pos);
                float d = sdBox(pos-_box1.xyz,_box1.www);
   float s = 1.0;
   for( int m=0; m<4; m++ )
   {
  float3 a = fmod( pos*s, _sphere1.x )-_sphere1.y;
      s *= 3.0;
      float3 r = _sphere1.z - _sphere1.w*abs(a);

      float c = sdCross(r)/s;
      d = max( d, -c );}
                //float s = DE(pos - _sphere1.xyz);
                //return opSmoothSubtraction(sphere1, box1, _thresh1);
                float p = sdOctahedron(pos - _sphere2.xyz, _sphere2.w);
                float b = sdBox(pos-(_sphere2.xyz - float3(0,_sphere2.w,0)),_sphere2.www);
            ...
            float3 shading(float3 p, float3 n){
                ...
                //result = _mainColor.rgb * (_lightCol * dot(-_lightDirection, n) * 0.5 + 0.5) * _lightIntensity;
                return result;
            }
